...
export const WalletDashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 min-h-[70vh]">
      ...
      <div className="bg-slate-900 border border-slate-800 text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
        ...
        <div className="space-y-2 relative z-10">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-md bg-indigo-500/25 text-indigo-350 text-xs font-semibold uppercase tracking-wider font-sans">
      ...
      {redeemSuccess && (
        <div className="bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-xl flex items-start gap-3 col-span-2">
      ...
      {loading ? (
        <div className="py-20 text-center text-sm text-gray-400 space-y-3 font-sans">
          <RefreshCw className="h-8 w-8 animate-spin mx-auto text-indigo-600" />
          <p className="text-xs text-gray-400">Syncing live balances and high-integrity ledger audits...</p>
        </div>
      ) : activeTab === 'wallet' ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            {/* Store Credit Cards */}
            <div className="bg-white rounded-xl border border-gray-250 p-6 space-y-4 shadow-sm">
              <h3 className="text-base font-bold text-gray-900 tracking-tight flex items-center gap-2 border-b border-gray-105 pb-3">
              ...
              {!data || data.storeCredit.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 border border-dashed border-gray-200 rounded-xl">
                  <p className="text-xs text-gray-500 font-sans">You currently do not have store credits registered in any currency.</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">Wallet balances are activated instantly when gift cards are redeemed.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {data.storeCredit.map((acct: any) => (
                    <div key={acct._id} className="bg-slate-50 border border-gray-200 rounded-xl p-4 flex flex-col justify-between space-y-3 shadow-2xs">
                      <div className="flex justify-between items-center">
                        <span className="font-mono text-[10px] text-gray-400 uppercase tracking-widest font-bold">Ledger Managed</span>
                      ...
                      <div className="text-[10px] text-gray-450 font-sans leading-tight">
            ...
            {/* Wallet Ledger Logs */}
            <div className="bg-white rounded-xl border border-gray-250 p-6 space-y-5 shadow-sm">
              <h3 className="text-base font-bold text-gray-900 tracking-tight flex items-center gap-2 border-b border-gray-105 pb-2">
              ...
              {!data || (data.history.storeCreditTransactions.length === 0 && data.history.giftCardTransactions.length === 0) ? (
                ...
                <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
                  ...
                  {data.history.storeCreditTransactions.map((tx: any) => (
                    <div key={tx._id} className="flex justify-between items-start border-b border-gray-100 pb-3 text-sm hover:bg-slate-50 p-2 text-left rounded-lg transition-colors">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase font-sans ${
                            tx.type === 'CREDIT_LOAD' ? 'bg-emerald-50 text-emerald-700' : tx.type === 'DEBIT_SPEND' ? 'bg-indigo-50 text-indigo-700' : 'bg-amber-50 text-amber-700'
                          }`}>
                            {tx.type === 'CREDIT_LOAD' ? 'Credit Loaded' : tx.type === 'DEBIT_SPEND' ? 'Checkout offsets' : 'Offsets Refund'}
                          </span>
                        </div>
                        <p className="text-xs text-gray-500 font-sans">{tx.notes}</p>
                        <div className="text-[10px] text-gray-400 font-mono">
                          {new Date(tx.transactionDate).toLocaleString()} 
                          {tx.orderId && ` • Order: ${tx.orderId.orderNumber || tx.orderId}`}
                        </div>
                      </div>
                      <div className={`font-mono font-bold text-right text-sm ${tx.amount > 0 ? 'text-emerald-600' : 'text-slate-800'}`}>
                  ...
                  {data.history.giftCardTransactions.map((tx: any) => (
                    <div key={tx._id} className="flex justify-between items-start border-b border-gray-100 pb-3 text-sm hover:bg-slate-50 p-2 text-left rounded-lg transition-colors">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase bg-purple-50 text-purple-700 font-sans mr-2">
                            Digital Certificate
                          </span>
                        </div>
                        <p className="text-xs text-gray-500 font-sans">{tx.note}</p>
                        <div className="text-[10px] text-gray-400 font-mono">
                          {new Date(tx.transactionDate).toLocaleString()} 
                          {tx.giftCard && ` • Card: ${tx.giftCard.code || 'CERTIFICATE'}`}
                        </div>
                      </div>
                      <div className={`font-mono font-bold text-right text-sm ${tx.amount > 0 ? 'text-purple-600' : 'text-slate-850'}`}>
                        {tx.amount > 0 ? '+' : ''}{tx.amount.toFixed(2)} {tx.currency}
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 border border-indigo-50 rounded-xl p-5 space-y-4 shadow-3xs">
              <h3 className="text-xs font-bold text-slate-800 tracking-wider font-sans flex items-center gap-1.5 uppercase border-b border-gray-200 pb-2">
              ...
              <form onSubmit={handleRedeemGiftCard} className="space-y-2">
                <input
                  ...
                  className="w-full text-center font-mono font-bold uppercase tracking-wider text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white text-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-520"
                />
                <button
                  type="submit"
              ...
              {!data || data.giftCards.length === 0 ? (
                ...
                <div className="space-y-3.5 max-h-[300px] overflow-y-auto font-sans pr-1">
                  {data.giftCards.map((card: any) => (
                    <div key={card._id} className="border border-gray-200 rounded-lg p-3 bg-slate-50 space-y-2 text-left shadow-4xs">
                      <div className="flex justify-between items-center">
                        <span className="font-mono text-xs font-bold text-gray-800 tracking-wide select-all">
                          {card.code}
                        </span>
                      ...
                      <div className="flex justify-between items-end">
                        <span className="text-[10px] text-gray-400">Claims Value:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-250 p-6 space-y-6 shadow-sm">
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1 bg-indigo-50 text-indigo-700 font-extrabold text-xs px-2.5 py-1 rounded-full font-mono uppercase tracking-wide">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="bg-slate-50 border border-gray-150 p-4 rounded-xl flex flex-col justify-between text-left">
                  <span className="text-[10px] text-gray-400 font-extrabold uppercase font-sans">Points Balance</span>
                  <p className="font-mono text-3xl font-extrabold text-indigo-650 mt-1">
                    {loyaltyData?.account?.pointsBalance || 0}
                  </p>
                  <span className="text-[10px] text-gray-450 font-sans mt-1">
                    ≈ ₹{((loyaltyData?.account?.pointsBalance || 0) * 0.84).toFixed(2)} Purchase discount
                  </span>
                </div>

                <div className="bg-slate-50 border border-gray-150 p-4 rounded-xl flex flex-col justify-between text-left">
                  <span className="text-[10px] text-gray-400 font-extrabold uppercase font-sans">Tier Multiplier</span>
                  <p className="font-mono text-3xl font-extrabold text-slate-800 mt-1">
                    {activeTierMultiplier}x
                  </p>
                  <span className="text-[10px] text-gray-450 font-sans mt-1 animate-pulse">
                ...
                <div className="bg-slate-50 border border-gray-150 p-4 rounded-xl flex flex-col justify-between text-left col-span-2 md:col-span-1">
                  <span className="text-[10px] text-gray-400 font-extrabold uppercase font-sans">Total Redeemed</span>
              ...
              {nextTierName ? (
                <div className="space-y-2.5 text-left bg-slate-50/60 p-4 rounded-xl border border-gray-105">
                  ...
                  <div className="flex justify-between items-center text-[10px] text-gray-450 font-sans">
                ...
                <div className="bg-indigo-50/40 border border-indigo-100 rounded-xl p-4 text-center space-y-1.5 flex items-center justify-center gap-3">
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <div className="bg-slate-950 border border-slate-800 rounded-lg p-2.5 flex justify-between items-center text-xs font-mono">
                    ...
                    <button 
                      ...
                      className="text-slate-400 hover:text-white hover:scale-105 transition-all p-1"
                    >
                  ...
                  <div className="pt-2 flex justify-between text-[11px] text-slate-450 border-t border-slate-800">
                    <span>Ambassador Level:</span>
                    <span className="font-semibold text-slate-200">Affiliate Link Partner</span>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-400 italic font-sans text-center">
            ...
            {!loyaltyData?.inboundReferral ? (
              <div className="bg-white border border-gray-250 rounded-xl p-5 space-y-4 shadow-3xs">
                ...
                <form onSubmit={handleClaimReferral} className="space-y-2">
                  <input
                    ...
                    className="w-full text-center font-mono font-bold uppercase tracking-wider text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white text-indigo-950 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                  <button
                    type="submit"
              ...
              <div className="bg-slate-50 border border-gray-200 rounded-xl p-5 space-y-3 shadow-3xs text-left">
            ...
            <div className="bg-white border border-gray-250 rounded-xl p-5 space-y-4 shadow-3xs text-left">
              <h3 className="text-xs font-bold text-slate-800 tracking-wider font-sans flex items-center gap-1.5 uppercase border-b border-gray-200 pb-2">
                <Users className="h-4.5 w-4.5 text-indigo-650" />
                Referrals Converted
              </h3>
              ...
              {!loyaltyData || !loyaltyData.referrals || loyaltyData.referrals.length === 0 ? (
                <p className="text-[11px] text-gray-400 italic font-sans text-center py-4">
                ...
                <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1 font-sans">
                  {loyaltyData.referrals.map((ref: any) => (
                    <div key={ref._id} className="border border-gray-150 rounded-lg p-2.5 bg-slate-50 space-y-1.5 shadow-4xs">
                      <div className="flex justify-between items-center">
